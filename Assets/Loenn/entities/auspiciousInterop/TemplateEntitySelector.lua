...
-- Exactly one node: the auspicioushelper template this entity feeds live
-- entities into.
entity.nodeLimits = {1, 1}
entity.nodeLineRenderType = "line"

-- Without this, Loenn only draws nodeSprite() output while the entity is
-- selected (default is "selected"). This matches TemplateBehaviorChain.
entity.nodeVisibility = "always"

-- Plain static icon for the main entity body - reuses auspicioushelper's real
-- "tgroup" icon asset. This is just a literal texture path, so it doesn't need
-- any code dependency on auspicioushelper, only that the mod (and therefore its
-- assets) is installed.
...
-- Explicit click/drag hitboxes for the main entity and its node. Without this,
-- Loenn falls back to a default hitbox that (for the node) isn't bounded the
-- way you'd expect, making it grab the node no matter where in the room you
-- click. Matches TemplateBehaviorChain.entity.selection exactly.
...
-- Dropped the aelperLib.template_entity_names check - a cross-mod require
-- almost certainly doesn't share module state with auspicioushelper's own
-- internal copy, so it was reading an empty table no matter what's actually
-- placed at the node. Instead, this just checks whether ANY entity sits
-- exactly on the node: not a guarantee it's a valid template, but enough
-- visual feedback to catch a misplaced/floating node, without depending on
-- auspicioushelper's Lua internals at all.
function entity.nodeSprite(room, entity, node, nodeIndex, viewport)
    ...
    return {
        ...
        -- drawableSprite.fromTexture("loenn/BalintHelper/template/group_error", {
        --     x = node.x, y = node.y,
        --     depth = -13001,
        -- }),
